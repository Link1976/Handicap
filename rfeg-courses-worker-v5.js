...
async function fetchPDF(license) {
  ...
  // Fetch sin redirect:follow — seguimos el redirect manualmente
  // para evitar que Cloudflare Workers pierda el body en redirects
  ...
  // Si hay redirect, seguirlo manualmente con el token
  let finalResp = resp;
  ...
  if (!finalResp.ok) {
    return new Response(`RFEG PDF error: ${finalResp.status}`, {
      status: finalResp.status,
      headers: { "Access-Control-Allow-Origin": "*" },
    });
  }

  // Pasar el body directamente como stream sin consumirlo
