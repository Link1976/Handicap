...
async function fetchPDF(license) {
  ...
  // Intento 1: sin token
  debug.steps.push({ step: "trying_no_auth", url: pdfUrl });
  const resp_noauth = await fetch(pdfUrl, {
    redirect: "manual",
    headers: {
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
  ...
  debug.steps.push({
    step: "resp_noauth",
    status: resp_noauth.status,
    content_length: resp_noauth.headers.get("content-length"),
    location: resp_noauth.headers.get("location"),
    content_type: resp_noauth.headers.get("content-type"),
  });

  // Intento 2: con token
  debug.steps.push({ step: "trying_with_token", url: pdfUrl });
  const resp_auth = await fetch(pdfUrl, {
    redirect: "manual",
    headers: {
      "Authorization": token,
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
  ...
  debug.steps.push({
    ...
    content_length: resp_auth.headers.get("content-length"),
    location: resp_auth.headers.get("location"),
    content_type: resp_auth.headers.get("content-type"),
  });

  // Usar la que tenga contenido
  let finalResp = resp_auth;
  if (resp_noauth.status === 200 && resp_noauth.headers.get("content-length") !== "0") {
    finalResp = resp_noauth;
    debug.steps.push({ step: "using_noauth" });
  } else if ([301,302,307,308].includes(resp_noauth.status)) {
    const loc = resp_noauth.headers.get("location");
    debug.steps.push({ step: "redirect_noauth", location: loc });
    if (loc) {
      finalResp = await fetch(loc, {
        headers: { "User-Agent": "Mozilla/5.0", "Accept": "application/pdf,*/*" },
      });
      debug.steps.push({ step: "resp_redirect", status: finalResp.status, content_length: finalResp.headers.get("content-length") });
    }
  } else if ([301,302,307,308].includes(resp_auth.status)) {
    const loc = resp_auth.headers.get("location");
    debug.steps.push({ step: "redirect_auth", location: loc });
    if (loc) {
      finalResp = await fetch(loc, {
        headers: { "Authorization": token, "User-Agent": "Mozilla/5.0", "Accept": "application/pdf,*/*" },
      });
      debug.steps.push({ step: "resp_redirect", status: finalResp.status, content_length: finalResp.headers.get("content-length") });
    }
  }
